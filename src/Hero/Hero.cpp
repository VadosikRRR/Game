...
Hero::Hero(string name_hero, fighter fighter_class_hero)
    : designation_('@'), xp_(0), money_(0), health_(START_MAX_HEALTH),
      max_health_(START_MAX_HEALTH),
      probability_of_hit_(START_PROBABILITY_OF_HIT), position_(Point(0, 0)),
      fighter_class_(fighter_class_hero),
      inventory_(Inventory(fighter_class_hero)) {
  CheckName(name_hero);
  PutFighterClassCharacteristics(fighter_class_hero);
}

char Hero::GetDesignation() { return designation_; }

unsigned int Hero::GetXp() { return xp_; }

unsigned int Hero::GetMoney() { return money_; }

unsigned int Hero::GetHealth() { return health_; }

unsigned int Hero::GetMaxHealth() { return max_health_; }

unsigned int Hero::GetMana() { return mana_; }

unsigned int Hero::GetMaxMana() { return max_mana_; }

unsigned int Hero::GetProtection() { return protection_; }

unsigned int Hero::GetProbabilityOfHit() { return probability_of_hit_; }

Point Hero::GetPosition() { return position_; }

unsigned int Hero::GetDamage() { return damage_; }

string Hero::GetName() { return name_; }

fighter Hero::GetFighterClass() { return fighter_class_; }

Inventory &Hero::GetInventory() { return inventory_; }
...
void Hero::ReduceProtection(unsigned int amount_protection) {
  protection_ =
      amount_protection > protection_ ? 0 : protection_ - amount_protection;
}

void Hero::ReduceProbabilityOfHit(unsigned int change_in_probability) {
  probability_of_hit_ = change_in_probability > probability_of_hit_
                            ? 0
                            : probability_of_hit_ - change_in_probability;
}
...
void Hero::IncreaseXp(unsigned int amount_xp) { xp_ += amount_xp; }

void Hero::IncreaseMoney(unsigned int amount_money) { money_ += amount_money; }

void Hero::IncreaseHealth(unsigned int amount_health) {
  health_ = amount_health + health_ > max_health_ ? max_health_
                                                  : amount_health + health_;
}
...
void Hero::IncreaseProbabilityOfHit(unsigned int change_in_probability) {
  probability_of_hit_ = change_in_probability + probability_of_hit_ > 100
                            ? 100
                            : change_in_probability + probability_of_hit_;
}
